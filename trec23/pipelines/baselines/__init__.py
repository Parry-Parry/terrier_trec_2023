...
def bm25_electra(model_name_or_path : str, dataset : str = None, index : Any = None, cut : int = 100, bm25_kwargs : Optional[dict] = {}, electra_kwargs : Optional[dict] = {}):
    ...
    return bm25 % cut >> load_electra(model_name_or_path, **electra_kwargs)

def splade_electra(splade_model_name_or_path : str, electra_model_name_or_path : str, cut : int = 100, splade_kwargs : Optional[dict] = {}, electra_kwargs : Optional[dict] = {}):
    splade = load_splade(splade_model_name_or_path, **splade_kwargs)
    electra = load_electra(electra_model_name_or_path, **electra_kwargs)
    return splade % cut >> electra
